...
void main() {
  testWidgets(
    'empty folder status reuses the side-pane list empty typography',
    (tester) async {
      await tester.pumpWidget(
        MaterialApp(
          theme: pbTheme(),
          home: const Scaffold(
            body: PbFileSelectStatus.empty(message: 'Nothing here yet'),
          ),
        ),
      );
  ...
  testWidgets(
    'file select dialog uses migrated shell and disables Add without a selection',
    (tester) async {
      var adds = 0;
      var closes = 0;
      ...
      final disabledAdd = find.ancestor(
        of: find.text('Add'),
  ...
  testWidgets(
    'file select dialog enables Add and routes room selection through the switcher',
    (tester) async {
      var adds = 0;
      String? selectedRoom;
      ...
      await tester.tap(
        find.byKey(const ValueKey('pb-file-select-room-switcher')),
      );
      await tester.pump();
  ...
  testWidgets(
    'file select breadcrumb preserves root and segment navigation callbacks',
    (tester) async {
      var rootPresses = 0;
      int? segmentIndex;
  ...
  testWidgets(
    'move destination dialog disables the current location and switches to copy',
    (tester) async {
            ...
            body: Stack(
              children: [
                PbFilesMoveDestinationDialog(
                  ...
                  fileBrowser: const Center(
                    child: Text('Live destination browser'),
                  ),
                  itemCount: 2,
                  canConfirm: false,
                  onRoomSelected: (_) {},
      ...
      expect(
        find.text('Choose a destination for 2 selected items.'),
        findsOneWidget,
      );
      expect(find.text('Live destination browser'), findsOneWidget);

      final moveButton = find.byWidgetPredicate(
        (widget) => widget is PbButton && widget.label == 'Move files to',
      );
      final disabledMove = find.ancestor(
        of: moveButton,
        matching: find.byType(IgnorePointer),
      );
      ...
      await tester.tap(
        find.byKey(const ValueKey('pb-files-copy-instead-toggle')),
      );
      await tester.pump();
      expect(
        find.byWidgetPredicate(
          (widget) => widget is PbButton && widget.label == 'Copy files to',
        ),
        findsOneWidget,
      );
  ...
  testWidgets('move destination dialog confirms the selected operation', (
    tester,
  ) async {
    bool? copyFilesInstead;
