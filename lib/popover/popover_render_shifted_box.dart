...
final class PopoverRenderShiftedBox extends RenderShiftedBox {
  ...
  void paint(PaintingContext context, Offset offset) {
    ...
    final pDirection = PopoverUtils.popoverDirection(attachRect, size, arrowHeight, direction);
    ...
    switch (pDirection) {
      case PopoverDirection.top:
        arrowRect = Rect.fromLTWH(arrowLeft, child!.size.height, arrowWidth!, arrowHeight);
      ...
      case PopoverDirection.left:
        arrowRect = Rect.fromLTWH(child!.size.width, arrowTop, arrowHeight, arrowWidth!);
    ...
    _paintShadows(context, transform, offset, pDirection, arrowRect, bodyRect);

    _pushClipPath(context, offset, PopoverPath(radius!).draw(pDirection, arrowRect, bodyRect), transform);
  }
  ...
  void _configureChildConstrains() {
    ...
    if (direction == PopoverDirection.top || direction == PopoverDirection.bottom) {
      childConstraints = BoxConstraints(maxHeight: constraints.maxHeight - arrowHeight).enforce(constraints);
    } else {
      childConstraints = BoxConstraints(maxWidth: constraints.maxWidth - arrowHeight).enforce(constraints);
    }
  ...
  void _configureChildOffset() {
    ...
    final childParentData = child!.parentData as BoxParentData?;
    if (pDirection == PopoverDirection.bottom) {
      childParentData!.offset = Offset(0, arrowHeight);
    } else if (pDirection == PopoverDirection.right) {
  ...
  void _pushClipPath(PaintingContext context, Offset offset, Path path, Matrix4 transform) {
    context.pushClipPath(needsCompositing, offset, offset & size, path, (context, offset) {
      context.pushTransform(needsCompositing, offset, transform, (context, offset) {
  ...
  void _transform(Matrix4 transform, Offset translation) {
    transform.translateByVector3(vm.Vector3(translation.dx, translation.dy, 0));
    transform.scaleByVector3(vm.Vector3(scale ?? 1.0, scale ?? 1.0, 1));
    transform.translateByVector3(vm.Vector3(-translation.dx, -translation.dy, 0));
  }
}
